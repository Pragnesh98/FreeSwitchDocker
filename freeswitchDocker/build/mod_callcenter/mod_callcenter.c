...
// user_exists id username domain
char *switch_find_user(switch_core_session_t *session, const char *user, const char *domain, char *key)
{
	...
	if (switch_xml_locate_user_merged(key, user, domain, NULL, &x_user, NULL) != SWITCH_STATUS_SUCCESS) {
		return switch_core_session_sprintf(session, "false");
	} else {
		switch_xml_free(x_user);
		return switch_core_session_sprintf(session, "true");
	}
	end:
...
char *user_data_function(switch_core_session_t *session, const char *user, const char *domain, char *type, char *key)
{
	...
	if (!zstr(data)) 
		return switch_core_session_sprintf(session, data);
	else 
		return NULL;
}
...
struct cc_queue {
	...
	char *queue_greeting;
	char *queue_timeout_action;
	char *queue_callback_enabled;
	char *queue_callback_extension;
...
static cc_queue_t *cc_load_queue(const char *queue_name, const char *dq_name);
...
switch_cache_db_handle_t *proxy_get_db_handle(void)
{
	switch_cache_db_handle_t *dbh = NULL;
	char *dsn;

	dsn = "pgsql://hostaddr=65.2.67.84 port=5432 dbname=kamailio user=postgres password=BngrpF2vAqIdn4MbTVTUVUO9I options=''";
...
switch_cache_db_handle_t *cc_get_db_handle(void)
{
	...
	if (!zstr(globals.odbc_dsn)) {
		...
		dsn = "pgsql://hostaddr=65.2.67.84 port=5432 dbname=ucall user=postgres password=BngrpF2vAqIdn4MbTVTUVUO9I options='' application_name='ucall'";
		//dsn = globals.dbname;
	}
...
cc_queue_t *queue_set_config(cc_queue_t *queue)
{
	...
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "queue-warp-up-time", SWITCH_CONFIG_STRING, 0, &queue->queue_wrap_up_time, "queue-warp-up-time", &queue->config_str_pool, NULL, NULL);
	
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "queue-timeout-action", SWITCH_CONFIG_STRING, 0, &queue->queue_timeout_action, "queue-timeout-action", &queue->config_str_pool, NULL, NULL);

	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "queue-greeting", SWITCH_CONFIG_STRING, 0, &queue->queue_greeting, "queue-greeting", &queue->config_str_pool, NULL, NULL);

	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "queue-callback-enabled", SWITCH_CONFIG_STRING, 0, &queue->queue_callback_enabled, "queue-callback-enabled", &queue->config_str_pool, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "queue-callback-extension", SWITCH_CONFIG_STRING, 0, &queue->queue_callback_extension, "queue-callback-extension", &queue->config_str_pool, NULL, NULL);
...
char *proxy_execute_sql2str(cc_queue_t *queue, switch_mutex_t *mutex, char *sql, char *resbuf, size_t len)
{
	char *ret = NULL;
...
static cc_queue_t *get_queue(const char *queue_name)
{
	...
	switch_mutex_lock(globals.mutex);
	if (!(queue = switch_core_hash_find(globals.queue_hash, queue_name))) {
		queue = cc_load_queue(queue_name,queue_name);
...
struct queue {
	char *call_center_queue_uuid ;
	char *domain_uuid;
	char *dialplan_uuid;
	char *queue_name;
	char *queue_extension;
	char *queue_strategy;
	char *queue_greeting;
	char *queue_timeout_action;
	char *queue_moh_sound;
	char *queue_record_template;
	char *queue_time_base_score;
	char *queue_max_wait_time;
	char *queue_max_wait_time_with_no_agent;
	char *queue_max_wait_time_with_no_agent_time_reached;
	char *queue_tier_rules_apply;
	char *queue_tier_rule_wait_second;
	char *queue_tier_rule_no_agent_no_wait;
	char *queue_discard_abandoned_after;
	char *queue_abandoned_resume_allowed;
	char *queue_tier_rule_wait_multiply_level;
	char *queue_cid_prefix;
	char *queue_announce_sound;
	char *queue_announce_frequency;
	char *queue_description;
	char *queue_wrap_up_time;
	char *queue_cc_exit_keys;
	char *team_uuid;
	char *agent_wrap_up_time;
//	char *queue_wrap_up_time;
	
	char *queue_callback_enabled;
	char *queue_callback_extension;
};
typedef struct queue queue_st;

static int cc_queue_callback_detail(void *pArg, int argc, char **argv, char **columnNames)
{
	queue_st *pbx = (queue_st *) pArg;
	int index = 0;
	
	for( index = 0; index < argc; index++ ) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ucall] %d : %s : %s\n", index, columnNames[index], argv[index]);
	}
	
	pbx->call_center_queue_uuid  = strdup(argv[0]);
	pbx->domain_uuid = strdup(argv[1]);
	pbx->dialplan_uuid = strdup(argv[2]);
	pbx->queue_name = strdup(argv[0]);
	pbx->queue_extension = strdup(argv[4]);
	pbx->queue_greeting = strdup(argv[5]);
	pbx->queue_strategy = strdup(argv[6]);
	pbx->queue_moh_sound = strdup(argv[7]);
	if (zstr(argv[7])) {
		pbx->queue_moh_sound = strdup("local_stream://default");
	}
	pbx->queue_record_template = strdup(argv[8]);
	pbx->queue_time_base_score = strdup(argv[9]);
	pbx->queue_max_wait_time = strdup(argv[10]);
	pbx->queue_max_wait_time_with_no_agent = strdup(argv[11]);
	pbx->queue_max_wait_time_with_no_agent_time_reached = strdup(argv[12]);
	pbx->queue_tier_rules_apply = strdup(argv[13]);
	pbx->queue_tier_rule_wait_second = strdup(argv[14]);
	pbx->queue_tier_rule_no_agent_no_wait = strdup(argv[15]);
	pbx->queue_timeout_action = strdup(argv[16]);
	pbx->queue_discard_abandoned_after = strdup(argv[17]);
	pbx->queue_abandoned_resume_allowed = strdup(argv[18]);
	pbx->queue_tier_rule_wait_multiply_level = strdup(argv[19]);
	pbx->queue_cid_prefix = strdup(argv[20]);
	pbx->queue_announce_sound = strdup(argv[21]);
	pbx->queue_announce_frequency = strdup(argv[22]);
	pbx->queue_cc_exit_keys = strdup(argv[23]);
	pbx->queue_description = strdup(argv[24]);
	pbx->queue_wrap_up_time = strdup(argv[25]);

	if(!zstr(argv[26])) {
		pbx->queue_callback_enabled = strdup(argv[26]);
	}
	if(!zstr(argv[27])) {
		pbx->queue_callback_extension = strdup(argv[27]);
	}
...
static cc_queue_t *cc_load_queue(const char *queue_name, const char *dq_name)
{
	...
	sql = switch_mprintf("select q.* from v_call_center_queues as q, v_domains as d  where d.domain_uuid = q.domain_uuid AND d.domain_enabled='true' AND q.call_center_queue_uuid='%s' ", dq_name);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG5, "[ucall] :  Find queue detail SQL : %s\n", sql);
	cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, cc_queue_callback_detail, &queuedata);
	
	switch_safe_free(sql);
	
	if(!zstr(queuedata.call_center_queue_uuid)) {
		switch_memory_pool_t *pool;
		int count=0;
		
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ucall] :  Loading Queue [ %s ] From DB.\n", queue_name);
		...
		if(!zstr(queuedata.queue_strategy)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "strategy", queuedata.queue_strategy);
			count++;
		}
	
		if(!zstr(queuedata.queue_moh_sound)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "moh-sound", queuedata.queue_moh_sound);
			count++;
		}
		if(!zstr(queuedata.queue_time_base_score)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "time-base-score", queuedata.queue_time_base_score);
			count++;
		}
		if(!zstr(queuedata.queue_max_wait_time)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "max-wait-time", queuedata.queue_max_wait_time);
			count++;
		}
		if(!zstr(queuedata.queue_max_wait_time_with_no_agent)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "max-wait-time-with-no-agent", queuedata.queue_max_wait_time_with_no_agent);
			count++;
		}
		if(!zstr(queuedata.queue_max_wait_time_with_no_agent_time_reached)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "max-wait-time-with-no-agent-time-reached", queuedata.queue_max_wait_time_with_no_agent_time_reached);
			count++;
		}
		if(!zstr(queuedata.queue_tier_rules_apply)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "tier-rules-apply", queuedata.queue_tier_rules_apply);
			count++;
		}
		if(!zstr(queuedata.queue_tier_rule_wait_second)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "tier-rule-wait-second", queuedata.queue_tier_rule_wait_second);
			count++;
		}
		if(!zstr(queuedata.queue_tier_rule_wait_multiply_level)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "tier-rule-wait-multiply-level", queuedata.queue_tier_rule_wait_multiply_level);
			count++;
		}
		if(!zstr(queuedata.queue_tier_rule_no_agent_no_wait)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "tier-rule-no-agent-no-wait", queuedata.queue_tier_rule_no_agent_no_wait);
			count++;
		}
		if(!zstr(queuedata.queue_discard_abandoned_after)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "discard-abandoned-after", queuedata.queue_discard_abandoned_after);
			count++;
		}
		if(!zstr(queuedata.queue_abandoned_resume_allowed)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "abandoned-resume-allowed", queuedata.queue_abandoned_resume_allowed);
			count++;
		}
		
		if(!zstr(queuedata.queue_announce_sound)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "announce-sound", queuedata.queue_announce_sound);
			count++;
		}
		
		if(!zstr(queuedata.queue_announce_frequency)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "announce-frequency", queuedata.queue_announce_frequency);
			count++;
		}
		
		if(!zstr(queuedata.queue_record_template)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "record-template", queuedata.queue_record_template);
			count++;
		}
		if(!zstr(queuedata.queue_greeting)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "queue-greeting", queuedata.queue_greeting);
			count++;
		}
		if(!zstr(queuedata.queue_timeout_action)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "queue-timeout-action", queuedata.queue_timeout_action);
			count++;
		}
		if(!zstr(queuedata.queue_wrap_up_time)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "queue-warp-up-time", queuedata.queue_wrap_up_time);
			count++;
		}
		if(!zstr(queuedata.queue_callback_enabled)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "queue-callback-enabled", queuedata.queue_callback_enabled);
			count++;
		}
		if(!zstr(queuedata.queue_callback_extension)) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "queue-callback-extension", queuedata.queue_callback_extension);
			count++;
		}
		...
		switch_mutex_init(&queue->mutex, SWITCH_MUTEX_NESTED, queue->pool);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "[ucall] :  Added queue %s\n", queue->name);
		switch_core_hash_insert(globals.queue_hash, queue->name, queue);
	}
...
static int cc_queue_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	if(!zstr(argv[0])) {
		char *queue_name = NULL;
		char *dq_name = switch_mprintf(argv[0]);
		
		queue_name = replaceWord(argv[0], " ", "-");
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "[ucall] :  Queue Name %s\n", queue_name);
		
		/*! Load Queue in FS memdory */
		cc_load_queue(queue_name, dq_name);
		
		switch_safe_free(dq_name);
	}
	return 0;
}

static int cc_load_queues_dynamic(void)
{
	char *sql = NULL;
	
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CONSOLE, "Trying to load DB queues\n");
	sql = switch_mprintf("select q.call_center_queue_uuid AS queue_name from v_call_center_queues as q, v_domains as d  where d.domain_uuid = q.domain_uuid  AND d.domain_enabled='true'");
	cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, cc_queue_callback, NULL /* Call back variables */);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG5, "[ucall] :  Loading Queues From DB SQL :\n%s\n", sql);
	switch_safe_free(sql);
	...
	if ((settings = switch_xml_child(cfg, "settings"))) {
		for (param = switch_xml_child(settings, "param"); param; param = param->next) {
			...
			if (!strcasecmp(var, "debug")) {
				...
				//globals.dbname = "pgsql://hostaddr=65.2.67.84 port=5432 dbname=ucall user=postgres password=BngrpF2vAqIdn4MbTVTUVUO9I options='' application_name='ucall'";
...
static void *SWITCH_THREAD_FUNC outbound_agent_thread_run(switch_thread_t *thread, void *obj)
{
	...
	switch_event_t *event = NULL;
	int bridged = 1;
				//const char *camp_uuid = NULL;
				//char camp_activity_uuid[256] = "";
	...
	if (!strcasecmp(h->agent_type, CC_AGENT_TYPE_CALLBACK)) {
		...
		if (h->record_template) {
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "cc_record_filename : %s\n",expanded);
			switch_channel_set_variable(member_channel, "cc_record_filename", expanded);
			switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "cc_record_filename", expanded);
			switch_channel_set_variable(member_channel, "cc_queue_record", expanded);
			//switch_safe_free(expanded);
		}
		
		if(!zstr(h->originate_string)) {
			char *sql = NULL;
			char res[256] = "";
			const char *domain_name = NULL;
			domain_name = switch_channel_get_variable(member_channel, "sip_h_X-context");
			sql = switch_mprintf("SELECT agent_id FROM v_call_center_agents WHERE call_center_agent_uuid='%s'", h->agent_name);
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG5, "  SQL QUERY :  [ %s ]\n", sql);
			cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
			switch_safe_free(sql);
			switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "sip_h_X-context", domain_name);
			
			if(!zstr(res)) {
			//	char cnt[10] = "";
				char *hold_music = NULL;
				char *user_exists = NULL;
				//sql = switch_mprintf("SELECT count(*) as cnt from location  WHERE username='%s' AND domain='%s' AND socket='tls:13.36.87.234:7443'",res, domain_name);
				//proxy_execute_sql2str(NULL, NULL, sql, cnt, sizeof(cnt));
				//switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG5, "  SQL QUERY :  [ %s ]\n", sql);
			//	switch_safe_free(sql);
			//	if (atoi(cnt) > 0) {
			//		switch_event_add_header(ovars, SWITCH_STACK_BOTTOM, "media_webrtc", "true");
			//	}
	//		switch_channel_set_variable(member_channel, "cc_agent", h->agent_name);


				switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "cc_agent_id", res);
				switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "media_webrtc", "true");
				user_exists = switch_find_user(member_session, switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")), domain_name, "user_exists");
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "  user_exists :  [ %s ]\n", user_exists);
				
				if(!zstr(user_exists) && !strcmp(user_exists, "true")) {
					hold_music = user_data_function(member_session, switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")), domain_name, "var", "hold_music");
					if(!zstr(hold_music)) {
						hold_music = replaceWord(hold_music, "${domain_name}", domain_name);
						
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, " caller hold_music :  [ %s ]\n", hold_music);
						switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "hold_music", hold_music);
					}
				}
				
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "Agent ID :  [ %s ]\n", res);
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "Caller ID Number :  [ %s ]\n", switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")));
				hold_music = user_data_function(member_session, res, domain_name, "var", "hold_music");
				if(!zstr(hold_music)) {
					hold_music = replaceWord(hold_music, "${domain_name}", domain_name);
					switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "hold_music", hold_music);
				}
				...
				//h->camp_uuid = switch_channel_get_variable(member_channel, "sip_h_X-CAMP-UUID");
				//sql = switch_mprintf("SELECT camp_activity_uuid FROM v_campaign_master WHERE camp_uuid = '%s'", h->camp_uuid);
				//switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "h->camp_uuid :: %s\n", h->camp_uuid);
				//cc_execute_sql2str(NULL, NULL, sql, camp_activity_uuid, sizeof(camp_activity_uuid));
				//switch_safe_free(sql);
				//switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "campaign_activity_uuid :: %s\n", camp_activity_uuid);
				//switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "cc_camp_activity_uuid", camp_activity_uuid);
		...
		if(!zstr(dialstr)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "originate_string [ %s ]\n", dialstr);
		}
		
		if(1) {
			char cc_bridge_start_epoch[256] = {'\0',};
			const char *cc_queue_wrap_up_time = NULL;
			char *execute_on_ring = NULL;
			execute_on_ring = switch_mprintf("lua ringtime.lua %s", switch_str_nil(switch_channel_get_variable(member_channel, "uuid")));
			sprintf(cc_bridge_start_epoch,"%" SWITCH_TIME_T_FMT,local_epoch_time_now(NULL));
			switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "cc_bridge_start_epoch", cc_bridge_start_epoch);
			switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "execute_on_ring",execute_on_ring);
			switch_channel_set_variable_printf(member_channel, "cc_bridge_start_epoch", cc_bridge_start_epoch);
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "cc_bridge_start_epoch :: %s\n", cc_bridge_start_epoch);
			switch_safe_free(execute_on_ring);
			
			cc_queue_wrap_up_time = switch_channel_get_variable(member_channel, "cc_queue_wrap_up_time");
			if(!zstr(cc_queue_wrap_up_time)) {
				switch_channel_set_variable_printf(member_channel, "sip_h_X-WRAPUP-TIME", cc_queue_wrap_up_time);
				switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "sip_h_X-WRAPUP-TIME", cc_queue_wrap_up_time);
				switch_event_add_header_string(ovars, SWITCH_STACK_BOTTOM, "cc_queue_wrap_up_time", cc_queue_wrap_up_time);
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_NOTICE, "sip_h_X-WRAPUP-TIME : [ %s ]\n", cc_queue_wrap_up_time);
			}
		}
...
static int agents_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	const char *agent_wrap_up_time = cbt->queue_wrap_up_time;
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Start Agent Callback \n");
	...
	//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "agent_wrap_up_time : %s\n", agent_wrap_up_time);
	if (! (atol(agent_last_bridge_end) < ((long) local_epoch_time_now(NULL) - atol(agent_wrap_up_time)))) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG5, "agent_wrap_up_time : %s\n", agent_wrap_up_time);
		contact_agent = SWITCH_FALSE;
	}
		...
		default: /* Go ahead, start thread to try to bridge these 2 caller */
			{
				...
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Start Outbound Therad \n");
...
static int members_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	cbt.queue_wrap_up_time = queue->queue_wrap_up_time;
		//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Queue strategy : %s\n", queue->strategy);
	...
	if (!strcasecmp(queue->strategy, "top-down")) {
		...
		//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Queue sql : %s\n",sql);
...
SWITCH_STANDARD_APP(callcenter_function)
{
	...
	if (argv[1]) {
		domain_uuid = argv[1];
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_WARNING, "domain_name = %s\n", domain_uuid);
	}
	...
	if (!zstr(queue->queue_callback_enabled) && !strcasecmp(queue->queue_callback_enabled,"t")) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_INFO, "Queue %s callback Enabled\n", queue_name);
		if (!zstr(queue->queue_callback_extension)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_INFO, "Queue callback execution : %s\n", queue->queue_callback_extension);
			switch_channel_set_variable(member_channel, "cc_callback_enabled", "true");
			switch_channel_set_variable(member_channel, "cc_callback_extension", queue->queue_callback_extension);
		}
	}

	if (!zstr(queue->queue_wrap_up_time)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_INFO, "Queue %s wrap up time : %s\n",queue_name, queue->queue_wrap_up_time);
		switch_channel_set_variable(member_channel, "sip_h_X-WRAPUP-TIME", queue->queue_wrap_up_time);
		switch_channel_set_variable(member_channel, "cc_queue_wrap_up_time", queue->queue_wrap_up_time);
	}
	...
	if (abandoned_epoch == 0) {
		...
		sql = switch_mprintf("INSERT INTO members"
				" (queue,instance_id,uuid,domain_uuid,session_uuid,system_epoch,joined_epoch,base_score,skill_score,cid_number,cid_name,serving_agent,serving_system,state)"
				" VALUES('%q','%q','%q','%q','%q','%q','%" SWITCH_TIME_T_FMT "','%d','%d','%q','%q','%q','','%q')",
				queue_name,
...
SWITCH_STANDARD_API(cc_config_api_function)
{
	...
	if (section && !strcasecmp(section, "timestamp")) {
		char cc_bridge_start_epoch[256] = {'\0',};
		sprintf(cc_bridge_start_epoch,"%" SWITCH_TIME_T_FMT,local_epoch_time_now(NULL));
		stream->write_function(stream, "%s", cc_bridge_start_epoch);
		goto done;
	}
	...
	if (section && !strcasecmp(section, "agent")) {
		...
		if (action && !strcasecmp(action, "load")) {
			if (argc-initial_argc < 1) {
				...
				const char *queue_name = argv[0 + initial_argc];
				cc_queue_t *queue = NULL;
				if ((queue = cc_load_queue(queue_name,queue_name ))) {
					stream->write_function(stream, "%s", "+OK\n");
				} else {
