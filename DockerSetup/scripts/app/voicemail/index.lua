...
--if the session exists
	if (session ~= nil) then
		...
		--get the domain_uuid
			...
			--freeswitch.consoleLog("notice","Domain_uuid :"..tostring(domain_uuid).."\n");
			if (domain_uuid ~= nil) then
		...
		--set the voicemail_dir
			...
			freeswitch.consoleLog("notice","[New] voicemail_dir :"..tostring(voicemail_dir).."\n");
...
--leave a message
	if (voicemail_action == "save") then
		...
		--set the variables
			...
			freeswitch.consoleLog("notice","[New] voicemail save\n");
		...
		--valid voicemail
			if (voicemail_uuid ~= nil) then
				...
				if (storage_type == "base64") then
						...
						-- build full path to file
							...
							freeswitch.consoleLog("notice","[New] full_path :"..tostring(full_path).."\n");
			...
			freeswitch.consoleLog("notice","Message_location : "..tostring(message_location).."\n");
					...
					for key,row in pairs(destinations) do
						...
						--save the message to the voicemail messages
							if (tonumber(message_length) > 2) then
								...
								table.insert(sql, "message_location");
								table.insert(sql, ") ");
